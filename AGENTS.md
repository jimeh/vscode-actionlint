...
- `toDiagnostics` — coordinate conversion (end_column: 1-based inclusive →
  ...
  from `actionlint.ruleSeverities` setting. Shellcheck errors are resolved
  to script-body positions via `resolveShellcheckRange` (parses SC message
  for line:col, maps through YAML block/inline scalar layout)
